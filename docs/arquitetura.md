...
- **Banco PostgreSQL**: armazena usuários, setores, dados do PDI, riscos, planos de ação, monitoramentos e histórico de alterações.
...
- **App `riscos`**: concentra PDI, macroprocessos, riscos, planos de ação, monitoramentos, histórico, dashboard e exportações.
...
- `Setor`: representa uma unidade administrativa da UFSM (alias público: `UnidadeOrganizacional`).
- `Usuario`: usuário customizado autenticado por SIAPE, vinculado a múltiplos setores. Possui campo `cargo` (`gestor` ou `gestor_adm`), campo `uuid` (identificador público exposto pela API — o ID interno sequencial nunca é retornado ao cliente), campo `sem_equipe_desde` (data em que o usuário ficou sem setor; após 7 dias, `is_active` retorna `False` e o acesso é bloqueado) e suporte a soft delete via campo `ativo`.
...
- `Risco`: registro principal do plano de risco. Possui campo `uuid` (identificador público). Suporta soft delete; ao ser desativado, propaga a desativação em cascata para `PlanoAcao` e `Monitoramento`.
- `PlanoAcao`: tratamento do risco, seguindo a lógica de ação/resposta. Possui campo `progresso` (0–100), calculado automaticamente pelo status da ação mas sobrescritível manualmente. Suporta soft delete.
- `Monitoramento`: acompanhamento contínuo de um risco. Suporta soft delete.
- `HistoricoPlano`: log imutável de alterações em um plano de risco. Registra `usuario_nome`, `data_hora` e `descricao` de cada ação (criação, edição, duplicação). Ordenado por `data_hora` e `id` decrescentes para garantir ordenação estável.
...
- `UsuarioSerializer` retorna usuário com seus setores e campo `sem_equipe_desde`.
...
- `PlanoAcaoSerializer` e `MonitoramentoSerializer` usam `SlugRelatedField(slug_field='uuid')` no campo `risco`, permitindo que o frontend envie o UUID do risco em vez do ID interno.
...
- gestão administrativa de usuários: listagem, soft delete e reativação (rotas usam UUID);
- perfil autenticado;
- recuperação de senha;
- membros de setor;
- adicionar ou remover membros da equipe (restrito a `gestor_adm` ou superusuário); ao remover o último setor de um usuário, registra `sem_equipe_desde`; ao adicionar qualquer setor, limpa `sem_equipe_desde`.

No app `riscos`, existem endpoints para:

- CRUD de riscos (rotas de detalhe usam UUID);
- duplicação de plano de risco (copia o risco e seus planos de ação);
- histórico de alterações do plano;
- listagem de desafios, objetivos e macroprocessos;
- busca expandida: evento, causa, consequência, macroprocesso, objetivo e responsável;
- ordenação por data, nível de risco (inerente e residual) e prazo;
- auto-progresso: atualização automática do campo `progresso` do `PlanoAcao` com base no `status`;
- dashboard consolidada;
- estatísticas;
- exportação em PDF e Excel (plano individual e lista filtrada);
- relatório gerencial em PDF (distribuição consolidada por categoria, unidade e nível);
- planos de ação e monitoramentos (suportam filtro por UUID do risco).
...
O método `is_active` do model `Usuario` implementa bloqueio automático por ausência de setor: se `sem_equipe_desde` estiver preenchido e tiver passado mais de 7 dias, `is_active` retorna `False`, causando `401` em qualquer requisição autenticada. Superusuários são isentos dessa regra.
...
Os IDs internos sequenciais nunca são expostos ao cliente. Todas as rotas de detalhe, atualização, exclusão, duplicação, histórico e exportação usam o campo `uuid` do model. Isso evita enumeração e exposição da estrutura interna do banco.
...
O `PlanoAcao` representa o tratamento do risco. Ele também é usado em filtros por período e na exibição de datas na dashboard. O campo `progresso` é atualizado automaticamente (0 para "Não iniciada", 50 para "Em andamento", 100 para "Concluída"/"Atrasada"), mas pode ser sobrescrito manualmente.
